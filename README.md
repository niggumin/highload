...
В разделе приводится описание алгоритмов функционирования блоков системы, способ реализации которых влияет на организацию данных, межсервисное взаимодействие и нагрузочные характеристики. Для MVP выбраны простые и предсказуемые алгоритмы, так как в сервисе доставки важнее стабильное время ответа и прозрачность эксплуатации, чем сложная оптимизация.
...
Для stateless-части используем Kubernetes. Stateful-компоненты выносим на выделенные серверы, чтобы не смешивать их с прикладным слоем.

## Пулы серверов

| Роль / пул                 | Конфигурация одного сервера                      | Кол-во | Размещение                | Что размещаем |
|:---------------------------|:------------------------------------------------|:-------|:--------------------------|:--------------|
| **L4 балансировщики**      | `4 vCPU, 8 GB RAM, 2x10GbE`                     | 4      | 2 в Москве, 2 в СПб       | `LVS + Keepalived` |
| **L7 балансировщики**      | `8 vCPU, 16 GB RAM, 2x10GbE`                    | 4      | 2 в Москве, 2 в СПб       | `Nginx` |
| **K8s control plane**      | `4 vCPU, 16 GB RAM, 200 GB NVMe`                | 3      | 2 в Москве, 1 в СПб       | `kube-apiserver`, `etcd`, control plane |
| **K8s worker general**     | `16 vCPU, 64 GB RAM, 500 GB NVMe`               | 8      | 4 в Москве, 4 в СПб       | gateway и sync API |
| **K8s worker async**       | `12 vCPU, 48 GB RAM, 500 GB NVMe`               | 4      | 2 в Москве, 2 в СПб       | notification, retry, kafka-consumer |
| **PostgreSQL**             | `32 vCPU, 128 GB RAM, 2x3.84 TB NVMe`           | 4      | primary + replicas в 2 ДЦ | `USER_PROFILE`, `ORDER`, `PAYMENT`, `PVZ`, `FILE_OBJECT` |
| **ClickHouse**             | `16 vCPU, 64 GB RAM, 2x7.68 TB NVMe`            | 2      | 1 в Москве, 1 в СПб       | `ORDER_EVENT` |
| **Redis / Sentinel**       | `8 vCPU, 32 GB RAM, 512 GB NVMe`                | 3      | 2 в Москве, 1 в СПб       | сессии и кеш |
| **Kafka brokers**          | `16 vCPU, 64 GB RAM, 2 TB NVMe`                 | 4      | 2 в Москве, 2 в СПб       | очереди и event streaming |
| **MinIO**                  | `8 vCPU, 32 GB RAM, 8 TB HDD + 512 GB NVMe`     | 4      | 2 в Москве, 2 в СПб       | бинарные файлы |
| **Monitoring / Logging**   | `8 vCPU, 32 GB RAM, 1 TB NVMe`                  | 2      | 1 в Москве, 1 в СПб       | `Prometheus`, `Grafana`, `ElasticSearch`, `Kibana` |

## Основные контейнеры в Kubernetes

| Deployment                | Реплики | Request CPU / RAM | Limit CPU / RAM | Назначение |
|:--------------------------|:--------|:------------------|:----------------|:-----------|
| **web-gateway**           | 4       | `500m / 512MB`    | `1 / 1GB`       | Вход для web-клиентов |
| **mobile-api-gateway**    | 4       | `500m / 512MB`    | `1 / 1GB`       | Вход для мобильных клиентов |
| **partner-api-gateway**   | 2       | `500m / 512MB`    | `1 / 1GB`       | Вход для юридических лиц |
| **auth-profile-service**  | 4       | `1 / 1GB`         | `2 / 2GB`       | Авторизация и профиль |
| **order-service**         | 6       | `1 / 2GB`         | `2 / 4GB`       | Создание и чтение заказов |
| **tracking-service**      | 4       | `1 / 2GB`         | `2 / 4GB`       | Трекинг и прием статусов |
| **pvz-service**           | 2       | `500m / 1GB`      | `1 / 2GB`       | Поиск ПВЗ |
| **route-tariff-service**  | 3       | `1 / 2GB`         | `2 / 4GB`       | Расчет маршрута и стоимости |
| **payment-service**       | 4       | `1 / 2GB`         | `2 / 4GB`       | Оплата и статусы платежа |
| **file-service**          | 2       | `500m / 1GB`      | `1 / 2GB`       | Метаданные файлов и доступ к MinIO |
| **notification-service**  | 3       | `500m / 1GB`      | `1 / 2GB`       | Push/email уведомления |
| **retry-dlq-worker**      | 2       | `500m / 1GB`      | `1 / 2GB`       | Повторы и разбор DLQ |

Такой состав дает запас по схеме `N+1` для stateless-слоя и позволяет независимо масштабировать синхронные и фоновые потоки.
