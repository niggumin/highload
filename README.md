...
1.  **Уровень L4 (Сетевой/Транспортный)**
...
erDiagram
    ...
    PVZ ||--o{ ORDER : "принимает/выдает"
    SESSION }o--|| USER_PROFILE : ""

    USER_PROFILE {
        id user_id PK "Идентификатор пользователя"
        string name "Имя"
        string surname "Фамилия"
        string patronymic "Отчество"
        string email "Почта"
        bool isEmailConfirmed "Подтверждена ли почта"
        int avatar "Аватарка"
        string phone "Телефон"
        string docs "Ссылка на файловы"
    }

    ORDER {
        id order_id PK "Идентификатор заказа"
        id sender_id FK "Идентификатор отправителя"
        id reciever_id FK "Идентификатор получателя"
        id sender_pvz_id FK "Идентификатор ПВЗ отправителя"
        id reciever_pvz_id FK "Идентификатор ПВЗ получателя"
        string status "Текущий статус"
        string route_data "Ссылка на этапы транспортировки"
        string photo "Ссылка на фото посылки"
        float price "Стоимость услуг"
        datetime created_at "Дата создания"
    }

    PVZ {
        id pvz_id PK "Идентификатор ПВЗ"
        string address "Адрес"
        string photo "Ссылка на фотографию"
        string info "Справочная информация"
        string schedule "Ссылка на график работы"
    }

    SESSION {
        string session_id PK "Идентификатор сессии"
        id user_id FK "Идентификатор пользователя"
...
| Сущность         | Описание данных (!!!еще не пересчитал!!!)      | Кол-во объектов | Объем данных | Чтение (Пик QPS)     | Запись (Пик QPS)          |
|:-----------------|:-----------------------------------------------|:----------------|:-------------|:---------------------|:--------------------------|
| **USER_PROFILE** | Данные профилей (ФИО, телефон, реквизиты)      | 25 000 000      | ~0,16 ТБ     | добавить авторизацию | добавить создание профиля |
| **ORDER**        | Данные заказов, статусы и маршруты (за 3 года) | 369 000 000     | ~24,39 ТБ    | ~575                 | ~25                       |
| **PVZ**          | Справочник пунктов выдачи заказов              | 8 600           | ~0,0006 ТБ   | ~210                 | << 1                      |
| **SESSION**      | Токены и состояния авторизации (528 Байт)      | 3 720 000 (DAU) | ~1,82 ГБ     | посчитать            | посчситать                |

## Требования к консистентности

Масштабирование чтения реализуется через репликацию (Master-Slave)
* **Строгая консистентность:** Обязательна для операций записи. Запросы направляются на Master ноду
* **Ограниченная консистентность:** Допускается для операций чтения трекинга и профилей со Slave нод
* **Кеширование:** Для часто запрашиваемых и редко меняющихся данных (например список ПВЗ) используется кеширование, что позволяет значительно снизить нагрузку на сервер

## Особенности распределения нагрузки по ключам

Приложение имеет небольшой объем хранилища, следовательно в шардинге нет смысла