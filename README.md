...
erDiagram
    ...
    USER_PROFILE {
        ...
        bool is_urid "Является ли пользователь юр лицом"
        int avatar "Аватарка"
    ...
    ORDER {
        ...
        int reciever_id FK "Идентификатор получателя"
        int sender_pvz_id FK "Идентификатор ПВЗ отправителя"
        int reciever_pvz_id FK "Идентификатор ПВЗ получателя"
...
* **Strict consistency:** Обязательна для операций записи (создание заказа, пользователя, пвз). Запросы направляются на Master ноду
* **Eventual consistency:** Допускается для операций чтения трекинга и профилей со Slave нод
